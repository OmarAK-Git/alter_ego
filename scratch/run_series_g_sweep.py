"""Series G sweep harness: fleet cohort drift + geo-velocity enabled (seed 42, thr=45)."""
...
DB_PATH = REPO_ROOT / "alter_ego_calibrate_series_g.db"
EVENTS_PATH = REPO_ROOT / "scratch" / "series_g_events.jsonl"
LABELS_PATH = REPO_ROOT / "scratch" / "series_g_ground_truth.jsonl"
METRICS_PATH = REPO_ROOT / "scratch" / "series_g_metrics.json"
ANOMALY_THRESHOLD = 45.0
GENERATOR_SEED = 42
CONFIG_VERSION = "2.2"

CONFIG_PATH = REPO_ROOT / "config" / "scoring_config.yaml"
CONFIG_BACKUP_PATH = REPO_ROOT / "config" / "scoring_config.yaml.series_g_backup"
...
def generate_series_g_mix() -> None:
...
def fleet_cohort_drift_decision_count(db) -> int:
    return db.execute(
        select(func.count())
        .select_from(DecisionRecordModel)
        .where(DecisionRecordModel.event_id == "COHORT_DRIFT")
    ).scalar_one()


def compute_series_g_extras(db, metrics: dict, threshold: float) -> dict[str, Any]:
    active_rows = list(
        db.execute(
    ...
    return {
        ...
        "fleet_cohort_drift_decision_count": int(fleet_cohort_drift_decision_count(db)),
    }
...
CHECKPOINT_PATH = REPO_ROOT / "scratch" / "series_g_checkpoint.json"
_SWEEP_FLAG_PATHS = [
...
def main() -> int:
    ...
    logger.info(
        "Running day-window pipeline against %s (fleet_drift + geo_velocity enabled)",
        DB_PATH,
    )
    ...
    try:
        ...
        extras = compute_series_g_extras(db, metrics, ANOMALY_THRESHOLD)
        ...
        metrics["fleet_cohort_drift_decision_count"] = extras[
            "fleet_cohort_drift_decision_count"
        ]
        s3 = merged_scenarios.get("scenario_3_subtle", {})
        s1 = merged_scenarios.get("scenario_1_sharp_misuse", {})
        metrics["s3_recall"] = s3.get("recall")
        metrics["s1_recall"] = s1.get("recall")
        ...
        payload = {
            ...
            "enabled_flags": {
                "cohort_gating_constants.fleet_drift_enabled": True,
                "drift_weights.geo_velocity": True,
            },
            "note": (
                "NOT CALIBRATED. Series G harness output (Phases 3–4: fleet cohort drift "
                "+ geo-velocity enabled). Compare S3 recall against residual-risk baseline "
                "0.667 (cross-series caution applies). Do not compare headline FP/P/R to "
                "Series A/B/C/D/E/F."
            ),
