"""Series D sweep harness: S1–S5 + tooling under R-INTERLOCK + D4 (seed 42, thr=45).

Clones Series C event mix with dual promotion_coverage metrics (ever + in-window N=5).
Do not claim CALIBRATED from this script alone. Full multi-minute execution is SD6.
"""
...
DB_PATH = REPO_ROOT / "alter_ego_calibrate_series_d.db"
EVENTS_PATH = REPO_ROOT / "scratch" / "series_d_events.jsonl"
LABELS_PATH = REPO_ROOT / "scratch" / "series_d_ground_truth.jsonl"
METRICS_PATH = REPO_ROOT / "scratch" / "series_d_metrics.json"
...
def generate_series_d_mix() -> None:
...
def compute_series_d_extras(db, metrics: dict, threshold: float) -> dict[str, Any]:
...
def main() -> int:
    ...
    try:
        ...
        extras = compute_series_d_extras(db, metrics, ANOMALY_THRESHOLD)
        ...
        payload = {
            ...
            "calibrated": False,
            "note": (
                "NOT CALIBRATED. Series D harness output (D4 time-axis). Do not compare "
                "FP/P/R to Series A, B, or C. Full sweep execution is SD6-gated."
            ),
