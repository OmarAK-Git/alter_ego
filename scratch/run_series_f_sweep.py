"""Series F sweep harness: cadence + volume dimensions enabled (seed 42, thr=45)."""
...
DB_PATH = REPO_ROOT / "alter_ego_calibrate_series_f.db"
EVENTS_PATH = REPO_ROOT / "scratch" / "series_f_events.jsonl"
LABELS_PATH = REPO_ROOT / "scratch" / "series_f_ground_truth.jsonl"
METRICS_PATH = REPO_ROOT / "scratch" / "series_f_metrics.json"
...
def generate_series_f_mix() -> None:
...
def per_dimension_drift_decomposition(db) -> dict[str, Any]:
    """H4-style dominance check: mean/max per-dimension delta across profile builds."""
...
def compute_series_f_extras(db, metrics: dict, threshold: float) -> dict[str, Any]:
    active_rows = list(
        db.execute(
...
CHECKPOINT_PATH = REPO_ROOT / "scratch" / "series_f_checkpoint.json"
_SWEEP_FLAG_PATHS = [
...
def main() -> int:
    ...
    logger.info("Running day-window pipeline against %s (cadence+volume enabled)", DB_PATH)
    ...
    try:
        ...
        extras = compute_series_f_extras(db, metrics, ANOMALY_THRESHOLD)
        ...
        payload = {
            ...
            "calibrated": False,
            "enabled_flags": {
                "drift_weights.cadence": True,
                "features.total_volume_delta": True,
                "drift_weights.total_volume_delta": True,
            },
            "note": (
                "NOT CALIBRATED. Series F harness output (Phases 1–2: cadence + volume "
                "dimensions enabled). Compare per_dimension_drift_decomposition for H4 "
                "dominance check. Do not compare headline FP/P/R to Series A/B/C/D/E."
            ),
