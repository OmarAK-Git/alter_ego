...
| DEBT-078 | B constants | `batch/profile_builder/builder.py:748–777` | Cadence drift dimension **cohort-constant as implemented**: `cadence_cov` saturates per role (SA≈1.0 for ~95%, median 1.000; humans=0.0 for all n=315 each); `norm_drift = raw_drift - cohort_median(role)` (`builder.py:868`) cancels entire contribution regardless of weight; absolute `cadence_cov` computed once outside `prev_profiles` loop (not Δ vs baseline); `max(0, 1−cv/0.3)` (DEBT-012) tuned for `scorer.compute_periodicity`'s 60-minute lookback but applied unchanged to 60–1440-minute build windows | S1 | **Code fix required (not implemented) before any sweep — two parts:** (1) **per-entity delta:** CoV(recent window)−CoV(baseline profile) inside the `prev_profiles` loop, not absolute regularity computed once outside it; (2) **timescale-appropriate divisor:** re-derive or parameterize the `cv/0.3` floor for build-window timescales in `compute_build_window_cadence_cov`, not blind reuse from the point feature; Series I `ws_cadence_2` aborted 2026-08-02 (n=1365 profiles / 65 entities) — w=2/5/10 arithmetically equivalent to control; no weight decision owed |

**ID range in use:** `DEBT-001` … `DEBT-078`. Next new item: `DEBT-079`.
